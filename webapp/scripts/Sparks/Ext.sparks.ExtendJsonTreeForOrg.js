 ...
 * @param config 需要的配置{}
 */
Ext.sparks.ExtendJsonTreeForOrg =  Ext.extend(Ext.lingo.JsonTree, {
	...
	initComponent: function() {
		Ext.applyIf(this, {
		 ...
		 Ext.sparks.ExtendJsonTreeForOrg.superclass.initComponent.call(this);
	...
	handelTreeClick: function(node, e) {
		var cmp = Ext.getCmp("user_editor_grid");
		//alert("1111=" + this.urlLoadData + "?dept=" + node.id);
		var dept = node.id;
		//alert("dept=" + dept);
        if (node.isLeaf()) {
			cmp.store.load({
